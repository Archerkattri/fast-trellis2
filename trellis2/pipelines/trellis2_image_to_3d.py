...
# Fast-TRELLIS acceleration helpers (token-carving spatial scores).
...
class Trellis2ImageTo3DPipeline(Pipeline):
    ...
    def from_pretrained(cls, path: str, config_file: str = "pipeline.json") -> "Trellis2ImageTo3DPipeline":
        ...
        # Auto-enable acceleration when the loaded samplers are the *_faster /
        # *_taylor variants, so coords_scores plumbing fires.
        pipeline.enable_faster = (
            type(pipeline.shape_slat_sampler).__name__.endswith('_faster')
            or type(pipeline.tex_slat_sampler).__name__.endswith('_faster')
        )
        pipeline.enable_voxel_visualization = False
    ...
    def sample_sparse_structure(
        ...
        if not self.enable_faster:
            return coords, None

        # Fast-TRELLIS: per-voxel spatial high-frequency scores for token
        # carving. Computed on the SAME (post-maxpool) occupancy grid via the
        # identical argwhere ordering used for `coords`, so coords_scores rows
        ...
        coords_scores = coords_scores.to(coords.device)
        if coords_scores.shape[0] != coords.shape[0]:
            print(f"[sparse-structure] coords_scores ({coords_scores.shape[0]}) "
                  f"!= coords ({coords.shape[0]}); token carving will be skipped.")
            coords_scores = None
        return coords, coords_scores
    ...
    def run(
        ...
        coords, coords_scores = self.sample_sparse_structure(
            cond_512, ss_res,
            num_samples, sparse_structure_sampler_params
        )
        # Fast-TRELLIS: inject SS-stage spatial scores into the shape SLaT
        # sampler so it can carve tokens. Only the non-cascade shape stage uses
        # `coords` directly (so scores stay index-aligned); cascade upsampling
        # re-derives coords, so carving auto-disables there and the easy
        # delta-cache still applies.
        if (self.enable_faster and coords_scores is not None
                and pipeline_type in ('512', '1024')
                and hasattr(self.shape_slat_sampler, 'set_coords_scores')):
            print("[fast-trellis] Injecting coords_scores into shape SLaT sampler")
            self.shape_slat_sampler.set_coords_scores(coords_scores)
        if pipeline_type == '512':
